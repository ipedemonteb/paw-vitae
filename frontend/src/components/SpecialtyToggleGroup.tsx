...
export default function SpecialtyToggleGroup({specialties, currentSpecialties, onValueChange, error, mutationPending}: SpecialtyToggleGroupProps) {
    ...
    return (
        <div className="flex flex-col gap-1">
            <ToggleGroup disabled={mutationPending} value={activeSpecialties} onValueChange={setActiveSpecialties} type="multiple" spacing={1.5} variant="outline" size="sm" className={`flex-wrap p-3  border-[0.5px]   bg-gray-50 rounded-xl overflow-clip  items-center ${error ? "border-(--danger)" : "border-gray-300"}`}>
