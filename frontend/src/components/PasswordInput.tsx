...
export function PasswordInput({ label, tooltip, id, required, className, error, hideLabel = false, icon, ...props }: PasswordInputProps) {
        ...
        <div className="space-y-2 w-full">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {tooltip && (
                        <div className="group relative">
                            ...
                            <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20">
                {tooltip}
              </span>
                        </div>
                    )}
            ...
            <div className="relative w-full">
                ...
                <input
                    ...
                    required={required}
                    className={cn(
                        "flex h-11 w-full rounded-lg border bg-(--gray-50) px-3 py-2 placeholder:text-(--gray-400) focus:outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-50 transition-colors pr-10",
                        icon && "pl-10",
                        error
                ...
                <button
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-(--gray-400) hover:text-(--gray-600) focus:outline-none rounded-md p-2"
                    tabIndex={-1}
                >
